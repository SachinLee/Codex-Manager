use codexmanager_core::rpc::types::{
    DailyUsageStatsParams, ModelDailyUsageStatSummary, ModelDailyUsageStatsResult,
};
...
pub(crate) fn read_model_daily_usage_stats(
    params: DailyUsageStatsParams,
) -> Result<ModelDailyUsageStatsResult, String> {
    let storage = open_storage().ok_or_else(|| "open storage failed".to_string())?;
    let (start_ts, end_ts) = resolve_day_bounds_ts(params.day_start_ts, params.day_end_ts)?;
    let items = storage
        .summarize_request_token_stats_by_model_between(start_ts, end_ts)
        .map_err(|err| format!("summarize model daily usage failed: {err}"))?
        .into_iter()
        .map(|item| ModelDailyUsageStatSummary {
            model: item.model,
        ...
        .collect();
    Ok(ModelDailyUsageStatsResult { items })
}
