...
use codexmanager_core::storage::{
    now_ts, ManagedModelV2, ManagedModelV2Upsert, ModelFastPolicyV2, ModelPriceV2, ModelRouteV2,
    Storage,
};
...
fn associate_account_models_with_storage(
    ...
    let models = storage
        .list_managed_models_v2(true)
        .map_err(|err| format!("read model catalog V2 failed: {err}"))?;
    let next_sort = models.iter().map(|model| model.sort_order).max().unwrap_or(0) + 1;
    let mut result = AggregateApiAssociateModelsResult::default();

    for (offset, (upstream_model, catalog_model)) in requested.iter().enumerate() {
        let existing = models
            .iter()
            .find(|model| model.slug.eq_ignore_ascii_case(catalog_model.as_str()));
        let mut model = existing.cloned().unwrap_or_else(|| {
            ...
            ManagedModelV2 {
                ...
                sort_order: next_sort + offset as i64,
                capabilities: json!({
        ...
        let route_model = if codexmanager_core::usage::is_luna_reserve_model(Some(upstream_model.as_str())) {
            model.slug.clone()
        } else {
            upstream_model.clone()
        };
        let route_exists = model.routes.iter().any(|route| {
            route.source_kind == ACCOUNT_POOL_SOURCE_KIND
                && route.source_id == ACCOUNT_POOL_SOURCE_ID
                && route.upstream_model.eq_ignore_ascii_case(route_model.as_str())
        });
        if route_exists {
            result.unchanged_routes.push(model.slug.clone());
            continue;
        }
        let inherited = model
            .routes
            .iter()
            .find(|route| route.source_kind == ACCOUNT_POOL_SOURCE_KIND && route.source_id == ACCOUNT_POOL_SOURCE_ID)
            .map(|route| (route.priority, route.weight))
            .unwrap_or((0, 1));
        model.routes.retain(|route| {
            !(route.source_kind == ACCOUNT_POOL_SOURCE_KIND && route.source_id == ACCOUNT_POOL_SOURCE_ID)
        });
        ...
        storage
            .upsert_managed_model_v2(&ManagedModelV2Upsert {
                previous_slug: was_existing.then(|| model.slug.clone()),
                model: model.clone(),
            })
