...
#[test]
fn request_log_estimate_does_not_double_count_cache_write_tokens() {
    let (storage, _) = prices();
    let without_cache_write = estimate_cost_usd_for_log(
        &storage,
        Some("gpt-5.4"),
        Some(1_000),
        Some(200),
    ...
    let with_cache_write = estimate_cost_usd_for_log(
        &storage,
        Some("gpt-5.4"),
        Some(1_000),
        Some(200),
