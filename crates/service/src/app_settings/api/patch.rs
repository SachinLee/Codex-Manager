...
pub(super) struct AppSettingsPatch {
    ...
    aggregate_api_session_affinity_enabled: Option<bool>,
...
pub(super) fn apply_app_settings_patch(patch: AppSettingsPatch) -> Result<(), String> {
    ...
    if let Some(enabled) = patch.aggregate_api_session_affinity_enabled {
        let _ = set_gateway_aggregate_api_session_affinity_enabled(enabled)?;
    }
