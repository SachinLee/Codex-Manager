...
fn clear_request_logs_hides_billed_rows_without_discarding_billing_audit_data() {
    ...
    let model_usage = storage
        ...
        .expect("summarize rolled-up model usage");
    let gpt_usage = model_usage
        .iter()
        .find(|usage| usage.model == "gpt-5.4-mini")
        .expect("rolled-up model usage remains visible to accounting");
    assert_eq!(gpt_usage.input_tokens, 12);
    assert_eq!(gpt_usage.cached_input_tokens, 2);
    assert_eq!(gpt_usage.output_tokens, 3);
