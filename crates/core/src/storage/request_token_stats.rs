...
impl Storage {
    ...
    pub fn summarize_request_token_stats_by_aggregate_api_between(
        ...
        let sql = format!(
            "WITH all_stats AS (
                SELECT COALESCE(
                           NULLIF(TRIM(aggregate_api_id), ''),
                           CASE
                               WHEN LOWER(TRIM(COALESCE(actual_source_kind, ''))) = 'aggregate_api'
                                   THEN NULLIF(TRIM(actual_source_id), '')
                           END
                       ) AS aggregate_api_id,
                       aggregate_api_supplier_name, aggregate_api_url,
                       input_tokens, cached_input_tokens, cache_write_input_tokens, output_tokens,
