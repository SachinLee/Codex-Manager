...
impl Storage {
    ...
    pub fn update_aggregate_api_upstream_protocol(
        &self,
        api_id: &str,
        upstream_protocol: Option<&str>,
    ) -> Result<()> {
        self.conn.execute(
            update_aggregate_api_upstream_protocol_sql(),
            (upstream_protocol, now_ts(), api_id),
        )?;
        Ok(())
    }
    ...
    pub(super) fn ensure_aggregate_apis_table(&self) -> Result<()> {
        ...
        self.ensure_column("aggregate_apis", "upstream_protocol", "TEXT")?;
...
fn map_aggregate_api_with_secrets_row(row: &Row<'_>) -> Result<AggregateApiWithSecrets> {
    Ok(AggregateApiWithSecrets {
        ...
        secret_value: row.get(28)?,
        balance_access_token: row.get(29)?,
    })
}
