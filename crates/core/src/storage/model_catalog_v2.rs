...
fn backfill_routes_from_model_source_mappings(conn: &Connection) -> Result<()> {
...
/// Grok 4.5 / Grok Build support low and high reasoning effort.
/// Only fills absent metadata so explicit catalog configuration stays intact.
fn backfill_missing_grok_reasoning_metadata(conn: &Connection) -> Result<()> {
    if !table_exists(conn, "models")? {
        return Ok(());
    }

    let mut stmt = conn.prepare(
        "SELECT id,default_reasoning_effort,capabilities_json FROM models
         WHERE lower(slug) LIKE 'grok-4.5%'
            OR lower(slug) = 'grok-build-latest'",
    )?;
    let rows = stmt.query_map([], |row| {
        Ok((
            row.get::<_, String>(0)?,
    ...
    let models = rows.collect::<Result<Vec<_>>>()?;
    drop(stmt);

    let now = now_ts();
    for (model_id, default_reasoning_effort, capabilities_json) in models {
        let mut capabilities = serde_json::from_str::<Value>(&capabilities_json)
            .ok()
            .filter(|value| value.is_object())
            .unwrap_or_else(|| Value::Object(Default::default()));
        let has_reasoning_efforts = capabilities
            .get("reasoning_efforts")
            .and_then(Value::as_array)
            .is_some_and(|efforts| !efforts.is_empty());
        let has_default_reasoning_effort = default_reasoning_effort
            .as_deref()
            .is_some_and(|effort| !effort.trim().is_empty());

        if has_reasoning_efforts && has_default_reasoning_effort {
            continue;
        }
        if !has_reasoning_efforts {
            capabilities["reasoning_efforts"] = serde_json::json!(["low", "high"]);
        }
        conn.execute(
            "UPDATE models SET default_reasoning_effort=CASE
                 WHEN default_reasoning_effort IS NULL OR TRIM(default_reasoning_effort)='' THEN 'high'
                 ELSE default_reasoning_effort
               END,
               capabilities_json=?2,updated_at=?3
             WHERE id=?1",
            params![model_id, capabilities.to_string(), now],
        )?;
    }
    Ok(())
}
...
impl Storage {
    ...
    pub(super) fn seed_missing_builtin_models_v2(&self) -> Result<()> {
        ...
        backfill_missing_grok_reasoning_metadata(&self.conn)?;
...
mod tests {
    ...
    fn seed_backfills_quota_aggregate_routes_and_missing_grok_prices() {
        ...
        grok.default_reasoning_effort = None;
        grok.capabilities = serde_json::json!({});
        ...
        storage
            .conn
            .execute(
                "UPDATE models SET default_reasoning_effort='low',
                   capabilities_json='{\"reasoning_efforts\":[\"low\"]}'
                 WHERE slug='grok-4.5';
        ...
        assert_eq!(grok_after.default_reasoning_effort.as_deref(), Some("low"));
        assert_eq!(
