...
old = '''fn migrate_legacy_routes(conn: &Connection) -> Result<()> {
    let now = now_ts();
...
new = r'''fn insert_route_if_missing(
    conn: &Connection,
    model_id: &str,
    source_kind: &str,
    source_id: &str,
...
fn migrate_legacy_routes(conn: &Connection) -> Result<()> {
    // First pass: explicit model_source_mappings for aggregate APIs (all enabled rows).
    // Main historically skipped 1:1 default mappings; custom branch primarily stored
    // assignments in quota_source_model_assignments, so both must be carried over.
    let now = now_ts();
    if conn
        .prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='model_source_mappings'")
        .and_then(|mut s| s.exists([]))
        .unwrap_or(false)
        && conn
            .prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='aggregate_apis'")
            .and_then(|mut s| s.exists([]))
            .unwrap_or(false)
    {
        let mut stmt = conn.prepare(
...
fn backfill_aggregate_routes_from_quota_assignments(conn: &Connection) -> Result<()> {
    let has_quota = conn
        .prepare(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='quota_source_model_assignments'",
        )
        .and_then(|mut s| s.exists([]))
        .unwrap_or(false);
    let has_apis = conn
        ...
        .unwrap_or(false);
    let has_routes = conn
        .prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='model_routes'")
        .and_then(|mut s| s.exists([]))
        .unwrap_or(false);
    let has_models = conn
        ...
        .and_then(|mut s| s.exists([]))
        .unwrap_or(false);
    if !has_quota || !has_apis || !has_routes || !has_models {
        return Ok(());
    }
...
fn backfill_missing_grok_prices(conn: &Connection) -> Result<()> {
    let has_models = conn
        ...
        .unwrap_or(false);
    let has_prices = conn
        .prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='model_prices'")
        .and_then(|mut s| s.exists([]))
        .unwrap_or(false);
    let has_tiers = conn
        .prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='model_price_tiers'")
        .and_then(|mut s| s.exists([]))
        .unwrap_or(false);
    if !has_models || !has_prices || !has_tiers {
        return Ok(());
    }
    ...
    let mut stmt = conn.prepare(
        "SELECT m.id,m.slug FROM models m
         ...
         WHERE p.price_status='missing'
           AND (
             m.slug LIKE 'grok-4.5%' COLLATE NOCASE
             OR m.slug = 'grok-build-latest' COLLATE NOCASE
           )",
    )?;
    let rows = stmt.query_map([], |row| {
...
if old not in text:
    raise SystemExit('old migrate_legacy_routes block not found')
text = text.replace(old, new, 1)

# Hook idempotent backfills into seed_missing_builtin_models_v2 and apply path
...
if old_seed not in text:
    raise SystemExit('seed_missing_builtin_models_v2 block not found')
text = text.replace(old_seed, new_seed, 1)

path.write_text(text, encoding='utf-8')
print('patched model_catalog_v2.rs ok')
