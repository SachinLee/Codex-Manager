...
- `CODEXMANAGER_CAPABILITY_ROUTING_MODE` (`off` / `observe` / `enforce`, 기본값 `enforce`; 환경 변수가 UI 저장값보다 우선함)
...
- `CODEXMANAGER_ROUTE_STRATEGY`
- `CODEXMANAGER_AGGREGATE_ZERO_DELIVERY_FAILOVER`: HTTP 200 응답의 `/v1/responses` 버퍼링 SSE에서 클라이언트에 출력이 전달되기 전에 terminal error가 발생한 경우 Aggregate API 후보를 전환합니다. 기본값은 `1`이며 `0`으로 설정하면 Aggregate API의 이 동작만 비활성화합니다. account pool 라우팅, capacity 복구, capability retry, reasoning guard 및 기존 5회 연속 실패 후 5분 cooldown 정책은 변경하지 않습니다.
