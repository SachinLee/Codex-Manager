...
- `CODEXMANAGER_CAPABILITY_ROUTING_MODE` (`off` / `observe` / `enforce`, по умолчанию `enforce`; переменная среды имеет приоритет над сохранённым значением UI)
...
- `CODEXMANAGER_ROUTE_STRATEGY`
- `CODEXMANAGER_AGGREGATE_ZERO_DELIVERY_FAILOVER`: включает переключение кандидата Aggregate API для буферизованных SSE-ошибок `/v1/responses`, когда HTTP 200 содержит терминальную ошибку до появления видимого для клиента вывода. По умолчанию `1`; значение `0` отключает только это поведение Aggregate API. Это не меняет маршрутизацию пула аккаунтов, восстановление после capacity, capability retry, reasoning guard или существующую политику охлаждения после пяти последовательных ошибок на пять минут.
