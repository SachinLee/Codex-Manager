...
- `CODEXMANAGER_AGGREGATE_TRANSPORT_RETRY_ATTEMPTS`: Aggregate API per-candidate retry budget for zero-delivery transport and Responses SSE server failures. Default `1` means the initial dispatch plus one retry; `0` disables same-candidate retries; a missing or invalid value falls back to `1`. It does not set candidate count, model fallback hops, health cooldowns, capacity/capability retries, or OMP retries. Restart the service after changing a process environment value.
- `CODEXMANAGER_AGGREGATE_ZERO_DELIVERY_FAILOVER`: enables Aggregate API candidate failover for buffered `/v1/responses` SSE failures that return an HTTP 200 terminal error before any client-visible output. Default `1`; set `0` to disable this Aggregate-only behavior. It does not change account-pool routing, capacity recovery, capability retries, reasoning guard, or the existing five-failures/five-minute cooldown policy.
