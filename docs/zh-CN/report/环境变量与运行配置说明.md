...
- `CODEXMANAGER_CAPABILITY_ROUTING_MODE`（`off` / `observe` / `enforce`，默认 `enforce`；环境变量优先于页面持久化值）
...
- `CODEXMANAGER_AGGREGATE_ZERO_DELIVERY_FAILOVER`：是否启用聚合 API 在缓冲式 `/v1/responses` SSE 中对“HTTP 200 但终态错误且客户端尚未收到输出”的候选切换，默认 `1`；设为 `0` 可关闭该聚合专属行为。不会改变账号池路由、容量恢复、能力重试、reasoning guard，或现有“连续失败 5 次后冷却 5 分钟”策略。
...
- `CODEXMANAGER_ROUTE_STRATEGY`
- `CODEXMANAGER_AUTO_COMPACT_ENABLED`：是否向 Codex 客户端发布模型目录中的 `auto_compact_token_limit`，默认 `0`。关闭时只禁用客户端自动 compact，不影响手动 `/v1/responses/compact` 和普通请求，也不会删除模型目录保存的阈值。
