...
| 模型目录 | `service_managed_model_add_aggregate_route_v2` | `apikey/managedModelAddAggregateRouteV2` | 仅管理员；参数：`slug`、`displayName`、`aggregateApiId`、`upstreamModel`；原子创建或复用本地模型，并添加或更新该聚合 API 显式路由 |
...
| 将发现的聚合模型添加到模型与路由 | `service_managed_model_add_aggregate_route_v2` / `apikey/managedModelAddAggregateRouteV2` |
