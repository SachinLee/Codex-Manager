...
export const EN_MODEL_CATALOG_MESSAGES: MessageCatalog = {
  ...
  启用路由: "Enable route",
  删除路由: "Delete route",
  上游全部不可用时的降级模型: "Fallback models when every upstream is unavailable",
  "按顺序尝试；当前模型的所有账号池与聚合 API 都不可用时才会切换。最多再试 3 个模型。": "Tried in order only after every account-pool and Aggregate API upstream for this model is unavailable. At most 3 fallback models are tried.",
  "未配置降级模型；上游全部不可用时直接返回错误。": "No fallback models configured; return an error when every upstream is unavailable.",
  添加降级模型: "Add fallback model",
  降级模型: "Fallback model",
  选择模型: "Select model",
  上移: "Move up",
  下移: "Move down",
